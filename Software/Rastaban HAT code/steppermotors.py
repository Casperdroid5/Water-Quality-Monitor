...
class STEPPERMOTORS():
    ...
    def SetMotorState(self, ENABLE: int):
        if ENABLE == 1:
            # low on enable pin to enable motordriver, tmc 2209
            self._pigpio.write(self._EnableGPIOPin, constants.OFF)
            self.state = State.MOTOR_ENABLED.name
            return self.state
        else:
            # high on enable pin to disable motordriver, tmc 2209
            self._pigpio.write(self._EnableGPIOPin, constants.ON)
...
if __name__ == "__main__":
    ...
    PrimaryStepper = STEPPERMOTORS(
        pi=pigpio.pi(), EnableGPIOPin=21, DirGPIOPin=23, StepGPIOPin=27)
    SecondaryStepper = STEPPERMOTORS(
        pi=pigpio.pi(), EnableGPIOPin=20, DirGPIOPin=22, StepGPIOPin=24)
    FocusStepper = STEPPERMOTORS(
        pi=pigpio.pi(), EnableGPIOPin=16, DirGPIOPin=26, StepGPIOPin=19)
    ...
    time.sleep(1)
    x = FocusStepper.SetMotorStep(400)
    print(x)

    print("Set Motor Direction Test")
    time.sleep(1)
    x = FocusStepper.SetMotorDir(0)
    print(x)
    ...
    print("Set Motor Step Test")
    time.sleep(1)
    # one rotation is 3200 pulses by microstepping 8. motor has 400 steps by full stepping
    x = PrimaryStepper.SetMotorStep(6400)
    print(x)
